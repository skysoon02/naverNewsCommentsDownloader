...
def newsListDownloader_MultiProcessing():
    with Pool(processes=number_of_process) as pool:
        return pool.map(newsListDownloader, (((channel['name'], categoryURL) for categoryURL in channel['categories']) for channel in targetChannelList))



def newsTitlesToURLs_MultiProcessing(newsList):
    with Pool(processes=number_of_process) as pool:
        return pool.map(newsURLDownloader, newsList)
...
def main():
    ...
    newsURLs = newsTitlesToURLs_MultiProcessing(newsList)
...
def debug():
    print(titleToURL("韓 '지금보다 더 최선 다할 것'", 448))
    return
